...
function eye(k,p,x,y,z,iris,side){
 const e=joint(p,side+'-eye',[x,y,z]);e.add(k.s(k.mat(0xeae2d2,.28),[.1,.1,.07],[0,0,0],side+'-eye-gloss'));
 const u=joint(e,side+'-pupil',[0,0,.058]);u.add(k.s(iris,[.057,.063,.02],[0,0,0],side+'-iris'));u.add(k.s(k.mat(0x171916,.2),[.029,.037,.012],[0,0,.017],side+'-dark-pupil'));u.add(k.s(k.mat(0xffffff,.18),[.018,.018,.008],[-.024,.028,.018],side+'-catchlight'));return[e,u];
}
function leg(k,p,name,x,z,y,a,b,paw,m,rear){
 ...
 const l=joint(u,name+'-lower-leg',[0,-a,0]);l.add(k.c(m,.10,b-.2,[1,1,.96],[0,-b/2,0],name+'-lower-coat'));l.add(k.s(m,paw,[0,-b-paw[1]*.7,.06],name+'-paw'));return[u,l];
}
function dog(kind,k){
 const golden=kind==='golden',root=new THREE.Group();root.name=golden?'golden-retriever':'french-bulldog';
 const rig=joint(root,kind+'-rig');rig.scale.setScalar(golden?.78:.77);const white=k.mat(golden?0xf4ead8:0xf4f0e5),coat=k.mat(golden?0xe2ceb0:0x191919),dark=k.mat(0x241610,.38),pink=k.mat(0xc77e76),iris=k.mat(0x644327,.24),base=golden?1.0:.72;
 const body=joint(rig,kind+'-body',[0,base,0]);body.add(k.s(white,golden?[.43,.43,.74]:[.37,.34,.43],[0,0,0],kind+'-torso'));
 ...
 if(golden)[[0,-.29],[-.15,-.22],[.15,-.22]].forEach(v=>body.add(k.s(coat,[.12,.19,.12],[v[0],v[1],.62],'golden-soft-chest-feather')));
 const neck=joint(body,kind+'-neck',[0,golden?.20:.13,golden?.49:.28]);neck.rotation.x=-.25;neck.add(k.c(white,golden?.24:.2,golden?.31:.17,[1.05,1,1],[0,.1,0],kind+'-neck'));
 const head=joint(neck,kind+'-head',[0,golden?.30:.20,-.03]);head.add(k.s(white,golden?[.38,.34,.39]:[.35,.31,.31],[0,0,0],kind+'-skull'));
 head.add(k.s(coat,golden?[.24,.13,.33]:[.27,.16,.17],[0,golden?-.12:-.06,golden?.36:.27],kind+'-muzzle'));head.add(k.s(dark,golden?[.105,.078,.07]:[.12,.065,.055],[0,golden?-.095:0,golden?.67:.45],kind+'-nose'));
 ...
 const jaw=joint(head,kind+'-jaw',[0,golden?-.22:-.14,golden?.35:.23]);jaw.add(k.s(coat,[.2,.07,.14],[0,0,.1],kind+'-lower-jaw'));jaw.add(k.s(pink,[.085,.018,.065],[0,-.075,.2],kind+'-tongue'));
...
function animate(a,v){
 ...
 a.body.position.y=a.base+Math.sin(t*(7+s*2))*.012-sit*.21;a.body.rotation.x=-sit*.4-ex*.06;a.head.rotation.set(-y*.16-ex*.075+sit*.05,x*.28+side*.19,0);a.jaw.rotation.x=ex*.34+s*.055;a.tail.rotation.y=Math.sin(t*(4+ex*8))*(.12+ex*.58+s*.12);a.tail.rotation.x=-sit*.12;
 ...
 a.ls.forEach((q,i)=>{const p=w*(i%2?-1:1),rear=i>1;q[0].rotation.x=p*a.stride*s*(rear?.78:1)+(rear?-sit: sit*.4);q[1].rotation.x=Math.max(0,-p)*a.stride*s*.56+(rear?sit*1.55:0)});
}
